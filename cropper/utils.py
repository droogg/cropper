...
def img_listextend (path, extention):
    img_list = []
...
def get_sorted_img_path(path: str, extention: list) -> list:
    path = norm_path(path)
    img_list = img_listextend(path, extention)
    img_list.sort(key=natural_keys)
    return img_list


def get_sorted_img_names(path: str, extention: list) -> list:
    path, img_fnl = norm_path(path), []
    img_list = get_sorted_img_path(path, extention)
    for i in range(len(img_list)):
        img_fnl.append(os.path.basename(img_list[i]))
    return img_fnl


def get_img_couple(path, extention):
    path = norm_path(path)
    img_fnl, img_couple_list = get_sorted_img_names(path, extention), []
